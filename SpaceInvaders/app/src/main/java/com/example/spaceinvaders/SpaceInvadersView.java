...
// This special constructor method runs
    public SpaceInvadersView(Context context, int x, int y) {
        ...
        shootID = sounds.load(context, R.raw.shoot, 1);
        invaderExplodeID = sounds.load(context, R.raw.invaderexplode, 1);
        damageShelterID = sounds.load(context, R.raw.damageshelter, 1);
        playerExplodeID = sounds.load(context, R.raw.playerexplode, 1);
        ohID = sounds.load(context, R.raw.oh, 1);
        uhID = sounds.load(context, R.raw.beep3, 1);
        ...
        while (playing) {
            ...
            if (!paused) {
                if ((startFrameTime - lastMenaceTime) > menaceInterval) {
                    if (uhOrOh) {
                        // Play Uh
                        sounds.play(uhID, 1, 1, 0, 0, 1);

                    } else {
                        // Play Oh
                        sounds.play(ohID, 1, 1, 0, 0, 1);
                    }
            ...
            for (int i = 0; i < numInvaders; i++) {
                if (invaders[i].getVisibility( )) {
                    if (RectF.intersects(bullet.getRect( ), invaders[i].getRect( ))) {
                        ...
                        sounds.play(invaderExplodeID, 1, 1, 0, 0, 1);
                        bullet.setInactive( );
                        score = score + 10;
                ...
                for (int j = 0; j < numBricks; j++) {
                    if (bricks[j].getVisibility( )) {
                        if (RectF.intersects(invadersBullets[i].getRect( ), bricks[j].getRect( ))) {
                            ...
                            sounds.play(damageShelterID, 1, 1, 0, 0, 1);
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < invadersBullets.length; i++) {
                if (invadersBullets[i].getStatus( )) {
                    if (RectF.intersects(playerShip.getRect( ), invadersBullets[i].getRect( ))) {
                        invadersBullets[i].setInactive( );
                        lives--;
                        sounds.play(playerExplodeID, 1, 1, 0, 0, 1);
