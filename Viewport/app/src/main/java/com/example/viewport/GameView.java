    ...
    private void update() {
        ...
        for (int i = 0; i < numBricks; i++) {
            if (!bricks[i].isClipped() && !bricks[i].isDestroyed()) {
                if (bricks[i].getRect().contains(player.getA().x, player.getA().y) || bricks[i].getRect().contains(player.getB().x, player.getB().y) || bricks[i].getRect().contains(player.getC().x, player.getC().y)) {
                    player.bump();          }       }   }
        ...
        player.getB().x > worldWidth || player.getC().x > worldWidth) {
...
// So we can override this method and detect screen touches.
    ...
    class HUD {
        ...
        HUD(int screenWidth, int screenHeight) {
            ...
            thrust = new Rect(screenWidth - buttonWidth - buttonPadding, screenHeight - buttonHeight - buttonPadding - buttonHeight - buttonPadding, screenWidth - buttonPadding, screenHeight - buttonPadding - buttonHeight - buttonPadding);
            ...
            switch (motionEvent.getAction( ) & MotionEvent.ACTION_MASK) {
                ...
                case MotionEvent.ACTION_DOWN:
                    if (right.contains(x, y)) {
                        ...
                        playerBullets[nextPlayerBullet].shoot(player.getA( ).x, player.getA( ).y, player.getFacingAngle( ));
                        nextPlayerBullet++;
